    ...
    if m.text == "المطورين":
        if sudo2(m):
            try:
                ...
                if lang is None:
                    await m.reply_text(f"◍ [Harp ](tg://user?id={super_sudoers[0]})\n" +
                                       f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n"
                                       "لا يوجد مطورين مرفوعين\n√",
                                       parse_mode=enums.ParseMode.MARKDOWN)
                else:
                    t = "\n◍ قائمة المطورين \n≪━━━━━━━━━━━━━≫\n" + f"◍ [Harp ](tg://user?id={super_sudoers[0]})\n" + \
                        f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n√"
                    for row in lang:
                        t = t + f"[{row[1]}](tg://user?id={row[0]})\n"
                    await m.reply_text(t, parse_mode=enums.ParseMode.MARKDOWN)
...
########################################################################################################################
    ...
    if m.text == (get_db_botname() or "حرب"):
        texting = ["اؤمر " + (get_db_botname() or "حرب") + " شتريد؟❤️🥺",
                   "اى يقلب " + (get_db_botname() or "حرب") + "❤️",
                   ...
                   "قلبى بينادى على " + (get_db_botname() or "حرب") + "😘",
                   "نعسان محدش يصحينى🙄"
                   ]
        await m.reply_text(random.choice(texting))
        return

    if m.text == "بوت غادر" or m.text == (get_db_botname() or "حرب") + " غادر":
        if secsudo(m):
            try:
                ...
                await m.reply_text(str(e) + "\n\n" +
                                   ...
                                   "[Harp](tg://user?id=super_sudoers[0])", parse_mode=enums.ParseMode.MARKDOWN)
                return
        else:
            await m.reply_text("◍ انت لست المطور\n√")
            return
...
########################################################################################################################
    ...
    if m.text == "السورس" or m.text == "سورس" or m.text == "يا سورس":
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [ InlineKeyboardButton ( "CH SOURCE", url=f"https://t.me/Source_Harp" )], 
        [InlineKeyboardButton("اضف البوت الي مجموعتك ✅",
                              url=f"https://t.me/{get_bot_information () [1]}?startgroup=new")],
        ])
        await m.reply_text("""
╭──── • ◈ • ────╮
么 [𝗦𝗢𝗨𝗥𝗖𝗘  Harp](t.me/Source_Harp)
么 [𝗔𝗦𝗞 𝗧𝗢 𝗠𝗘](t.me/Source_Harp)
么 [Harp](t.me/H_A_R_P_M)
╰──── • ◈ • ────╯
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
    ...
    if m.text == "حرب" or m.text == "حرب":
        s = await c.get_users("7082813679")
        ss = (await c.get_chat("7082813679")).bio
        sender_name = f"[{m.from_user.first_name}](tg://user?id={m.from_user.id})"
        name_chat = m.chat.title
        id_chat = m.chat.id
        if m.chat.type == enums.ChatType.SUPERGROUP:
        ...
        try:
            await c.get_chat_photos("7082813679", limit=1).__anext__()
        except:
            await m.reply_text(f"{ss}", reply_markup=keyboard),
            await c.send_message("7082813679", f"❤️╖ نداء لك ايها المبرمج\n📟╢ بواسطة {sender_name}\n📆╢ يوم *{today}*\n🕑╢ الساعه *{clock}*\n💌╢ اسم الجروب {name_chat}\n🔰╢ ايدي الجروب *{id_chat}*\n⚙️╢ عدد اعضاء الجروب *{num_member}*\n⛓╢ رابط المسج {message_link}\n🔍╜ الرابط {link_group}", parse_mode=enums.ParseMode.MARKDOWN)
        else:
            async for photo in c.get_chat_photos("7082813679", limit=1):
